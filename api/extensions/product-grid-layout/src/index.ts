...
export default defineLayout({
  ...
  setup(props, { emit }) {
    ...
    const fields = computed({
      get: () => {
        // Always include attributes field to get junction data
        const baseFields = layoutQuery.value?.fields || ['*'];
        if (!baseFields.includes('*') && !baseFields.includes('attributes')) {
          return [...baseFields, 'attributes.attribute_id', 'attributes.value', 'attributes.attribute_id.id', 'attributes.attribute_id.code', 'attributes.attribute_id.label'];
        }
        return baseFields;
      },
      set: (val) => {
